...
- 一次提问上传多个文件时，自动合并为一个文件夹上传，所有文件共享同一 user_tag 与文件管理链接
...
当用户一次提问中上传多个文件时，脚本会自动将它们**合并为一个文件夹**（走文件夹上传接口），归到**同一个 user_tag** 下，共享同一个 `manage_url`，用户只需管理一个链接。调用方式：把所有文件路径作为参数一次性传给脚本。
...
合并规则：
- 文件夹内文件默认使用各自文件名；重名文件自动追加 `-1`、`-2` 后缀去重
- 全部文件在同一目录时，文件夹名取该目录名；否则为通用名「多文件上传」
- 类型未开放的文件自动跳过，记录在输出的 `skipped` 数组中，不影响其他文件

脚本输出聚合 JSON，含 `mode: "folder"`、`folder_key`、`folder_url`、`files`（每项含 `url` 直链与 `rel_path`）、`skipped` 以及共用的 `manage_url`。

> ⚠️ 不要对每个文件分别调用 `upload.sh`，否则会生成多个 user_tag、得到多个管理链接，无法归档。
>
> 单文件直接调用 `bash upload.sh "<文件>"` 即可，兼容（走单文件上传接口）。详见 SKILL.md「上传流程」。
