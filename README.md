...
- 🛡️ **管理后台**：基于 CRON\_SECRET 鉴权，查看所有文件、删除文件、一键清理过期文件
...
- **存储**：Cloudflare R2 → 公网直链输出，cacheControl 与 EXPIRE\_HOURS 联动
...
**原因**：输入的 CRON\_SECRET 与 GitHub Secret 中配置的不一致。
...
| `filename`   | TEXT NOT NULL                   | 文件名（时间戳随机名.扩展名）        |
...
| `idx_images_filename`  | `filename`  | 文件名查询  |
...
1. **上传**：前端直接调用 Worker API → Worker 校验 → 存入 R2 + D1
2. **访问**：直接返回 R2 公网直链，cacheControl 与 EXPIRE\_HOURS 联动
3. **查询**：通过 D1 数据库按 user\_tag 查询，仅返回未过期记录
4. **清理**：Cron 每小时查询 D1 中过期记录 → 删除 R2 文件 + D1 记录
5. **管理**：X-Cron-Secret 鉴权 → 查看全部/删除/清理
...
4. 创建 D1 数据库：`wrangler d1 create flyimg-db`，将 database\_id 填入 wrangler.toml
