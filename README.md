...
> **重要提示**：由于中国大陆的网络环境特殊性，Cloudflare 默认的 `workers.dev`、`pages.dev` 和 `r2.dev` 子域名在大陆可能无法正常访问。如果你希望在中国大陆使用此图床，**必须为 R2、Worker 和 Pages 绑定自定义域名**。

### 为什么需要自定义域名？

Cloudflare 的免费子域名（如 `*.workers.dev`、`*.pages.dev`、`*.r2.dev`）在部分地区（尤其是中国大陆）可能被 DNS 污染或 SNI 阻断，导致无法访问。绑定你自己的域名可以绕过这些限制，确保稳定访问。
...
6. 更新 GitHub Secret `WORKER_URL` 为新的自定义域名地址（如 `https://img.example.com`）
7. 回到 GitHub 仓库 → **Actions** → **Run workflow** 重新触发部署
...
1. 登录 [Cloudflare Dashboard](https://dash.cloudflare.com/)
2. 左侧菜单 → **Workers 和 Pages** → 点击 `flyimg-frontend`（或你的 Pages 项目名称）
3. 点击 **自定义域** → **设置自定义域**
4. 输入你的前端子域名（如 `flyimg.example.com` 或 `images.example.com`）
...
|------|-----------|-------------------|
| Worker API | `img-api.example.com` | `WORKER_URL=https://img-api.example.com` |
| R2 图片存储 | `img-cdn.example.com` | `R2_PUBLIC_DOMAIN=https://img-cdn.example.com` |
| Pages 前端 | `img.example.com` | 无需更新，直接访问新域名 |
...
本项目设计时充分考虑了 Cloudflare 的免费额度限制，确保日常使用零成本。以下是各项服务的免费额度详细说明：

### Workers（API 服务）

| 项目 | 免费额度 | 本项目典型用量 |
|------|---------|--------------|
| 请求次数 | 100,000 次/天（UTC 时间） | 上传 10 次/天 = 20 次请求（上传 + 查询） |

**说明**：
- 每次上传文件会产生 1 次 POST 请求
- 每次查看我的图片会产生 1 次 GET 请求
- Cron 定时任务每小时执行 1 次清理任务 = 24 次/天
- 统计页面请求极少，忽略不计

**估算**：
- **轻度使用**（每天上传 5 次）：约 35 次/天 = **每月约 1,050 次**
- **中度使用**（每天上传 20 次）：约 64 次/天 = **每月约 1,920 次**
- **重度使用**（每天上传 50 次）：约 124 次/天 = **每月约 3,720 次**

**结论**：日常使用几乎不可能用完 100,000 次/天的额度。

### R2（对象存储）

| 项目 | 免费额度 | 本项目典型用量 |
|------|---------|--------------|
| 存储空间 | 10GB | 100 张图片 × 平均 2MB = 200MB |
| A 类操作（写入） | 1,000,000 次/月 | 每上传 1 次 = 1 次写入 |
| B 类操作（读取） | 10,000,000 次/月 | 图片直链访问不经过 Worker，直接通过 R2 公网地址访问 |
| 出站流量 | **无限量**（免费） | 所有图片访问都不消耗 Worker 请求和流量 |

**说明**：
- 存储空间建议设置 `MAX_STORAGE_SIZE` 为 1000MB（1GB），预留 90% 的免费额度
- 图片直链直接访问 R2，不经过 Worker，这是项目架构的优势
- 过期文件被自动清理后会释放存储空间

**估算**：
- 单张图片平均 2MB：可存储约 **5,000 张图片**
- 单张图片平均 5MB（高清）：可存储约 **2,000 张图片**
- 每天上传 10 张 × 2MB = 20MB/天 = 约 **500 天**达到 10GB 上限

**结论**：配合自动清理功能，存储空间几乎不会用满。
...
| 写入 | 100,000 次/天 | 每上传 1 次 = 1 次写入，Cron 删除 = 批量写入 |

**说明**：
- 每条元数据记录约 200 字节，5GB 可存储约 **2500 万条记录**
- 个人使用每天查询约 20 次，消耗约 20 次读取
- Cron 任务每小时执行 1 次删除操作，约 24 次写入/天

**估算**：
- **轻度使用**（每天 5 次上传 + 10 次查询）：约 15 次读取 + 5 次写入
- **中度使用**（每天 20 次上传 + 50 次查询）：约 70 次读取 + 20 次写入
- **重度使用**（每天 100 次上传 + 200 次查询）：约 300 次读取 + 100 次写入

**结论**：D1 的免费额度对个人项目来说是**完全无限制**的。

### Pages（前端托管）

| 项目 | 免费额度 | 本项目典型用量 |
|------|---------|--------------|
| 构建次数 | 500 次/月 | 每次部署 = 1 次构建 |
| 带宽 | 无限 | 前端页面访问消耗 |
| 存储空间 | 无限 | 静态 HTML 文件约 50KB |

**说明**：
- 每次部署（包括自动和手动）会消耗 1 次构建额度
- 500 次/月相当于每天可以部署 16 次，足够日常开发使用
- 前端页面是纯静态 HTML，没有后端处理，访问不消耗额外额度

**结论**：除非每天频繁部署十几次，否则不会用完构建额度。

### 用量总结

| 使用场景 | Workers | R2 存储 | D1 读取 | D1 写入 | Pages 构建 |
|---------|---------|---------|---------|---------|-----------|
| **个人轻度**（每天 5 张） | 0.035% | 2% | 0.0003% | 0.005% | <1 次 |
| **个人中度**（每天 20 张） | 0.064% | 4% | 0.0014% | 0.02% | <1 次 |
| **小团队**（每天 50 张） | 0.124% | 10% | 0.006% | 0.1% | <1 次 |

**总体结论**：对于个人或小团队使用，本项目在 Cloudflare 免费额度内完全可以稳定运行，**零成本**。只要设置合理的 `EXPIRE_HOURS` 和 `MAX_STORAGE_SIZE`，图片会自动清理，持续使用也不会超出免费限制。
...
本项目使用 Cloudflare D1（基于 SQLite 的无服务器数据库）存储图片元数据。初始化脚本位于 `migrations/0001_init.sql`。

### 表结构

**`images` 表**：存储所有上传的图片元数据
...
- `idx_images_user_tag`：按用户查询图片时使用
- `idx_images_expire_at`：定时任务查找过期图片时使用
- `idx_images_filename`：按文件名精确查询时使用

### 部署时自动应用

数据库迁移脚本在 GitHub Actions 部署时**自动执行**，无需手动操作。部署流程会：

1. 读取 `migrations/0001_init.sql`
2. 通过 Wrangler CLI 创建或更新 D1 数据库
3. 执行 SQL 脚本初始化表结构
...
**进入数据库交互式查询**：
```bash
npx wrangler d1 execute flyimg-db --local
```

**执行 SQL 查询**：
```bash
npx wrangler d1 execute flyimg-db --command "SELECT * FROM images LIMIT 10;"
```

**导入本地 SQL 文件**：
```bash
npx wrangler d1 execute flyimg-db --file=migrations/0001_init.sql
```

### 数据库配置

D1 数据库配置在 `wrangler.toml` 文件中：

```toml
[[d1_databases]]
binding = "DB"
database_name = "flyimg-db"
database_id = "你的数据库ID"
```

部署时，`database_id` 会自动创建并更新。
