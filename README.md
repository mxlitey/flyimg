...
| `WORKER_URL` | 你的 Worker 地址 | 部署完成后 Dashboard 中查看 |

> **如何获取 WORKER_URL？**
> 首次部署 Worker 成功后，进入 Dashboard → Workers & Pages → flyimg-worker，复制页面地址即可。
> 格式类似：`https://flyimg-worker.你的子域名.workers.dev`
...
- **前端页面地址**：`https://flyimg.你的子域名.pages.dev`（Cloudflare Dashboard → Workers & Pages → Pages → flyimg）
