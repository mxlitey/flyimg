...
- 👤 **用户标识系统**：通过 user\_tag 标识用户，支持查看个人已上传的未过期图片
...
- **API调用**：前端直接调用 Worker API（地址在部署时自动注入 config.js）
...
| Secret 名称              | 默认值                        | 说明                               |
| ---------------------- | -------------------------- | -------------------------------- |
| `WORKER_CUSTOM_DOMAIN` | 无（使用 workers.dev）          | Worker 自定义域名，如 `img.example.com` |
...
| 角色   | 访问方式         | 权限                           |
| ---- | ------------ | ---------------------------- |
| 普通用户 | 免登录          | 上传图片、通过 user\_tag 查看自己的未过期图片 |
...
| `WORKER_CUSTOM_DOMAIN` | 无        | Worker 自定义域名（需域名已托管在 Cloudflare）            |
...
4. **查询**：通过 D1 数据库按 user\_tag 查询，仅返回未过期记录
...
Worker 使用 Cloudflare 全球 CDN，默认即可在全球访问。如果遇到连接问题，可以为 Worker 绑定自定义域名并通过 CDN 优化访问。
